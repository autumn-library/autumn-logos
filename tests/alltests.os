...
&Тест
Процедура Должен_ПроверитьПередачуНастроекАппендеровВИерархииЛогов() Экспорт
	...
	// Когда 
	КонтейнерЛога = Поделка.НайтиЖелудь("КонтейнерСДочернимХитрымАппендером");
	Аппендер = Поделка.НайтиЖелудь("ХитрыйАппендер");
	...
	// Тогда
	Ожидаем.Что(Сообщения.Количество(), "Дочерний логгер получил аппендер из родительских настроек").Равно(1);
	Ожидаем.Что(Сообщения[0], "текст сообщения дочернего логгера").Равно("ИНФОРМАЦИЯ - Сообщение1");

КонецПроцедуры

&Тест
Процедура Должен_ПроверитьНаследованиеАппендеровПриСтроковойНастройкеДочернегоЛоггера() Экспорт
	...
	// Когда
	КонтейнерЛога = Поделка.НайтиЖелудь("КонтейнерСДочернимХитрымАппендеромСтроковыйУровень");
	Аппендер = Поделка.НайтиЖелудь("ХитрыйАппендер");
	...
	// Тогда
	Ожидаем.Что(Сообщения.Количество(), "Дочерний логгер со строковой настройкой уровня получил аппендер из родительских настроек").Равно(1);
	Ожидаем.Что(Сообщения[0], "текст сообщения дочернего логгера со строковой настройкой").Равно("ИНФОРМАЦИЯ - Сообщение1");

КонецПроцедуры

&Тест
Процедура Должен_ПроверитьНаследованиеАппендеровОтRootLogger() Экспорт
	...
	// Когда
	КонтейнерЛога = Поделка.НайтиЖелудь("КонтейнерСЛогомОтРутЛоггера");
	Аппендер = Поделка.НайтиЖелудь("ХитрыйАппендерRoot");
	...
	// Тогда
	Ожидаем.Что(Сообщения.Количество(), "Логгер без прямой настройки получил аппендер из rootLogger").Равно(1);
	Ожидаем.Что(Сообщения[0], "текст сообщения логгера с аппендером от rootLogger").Равно("ИНФОРМАЦИЯ - Сообщение1");
