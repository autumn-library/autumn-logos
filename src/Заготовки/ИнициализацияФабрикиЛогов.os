...
Процедура ИнициализацияЛогов()
	...
	Если ЗначениеЗаполнено(НастройкиЛогированияСтрокой) Тогда
		...
		НастройкиЛогирования = Рефлектор.ПолучитьСвойство(Логирование, "мНастройкиЛогирования");
	...
	Для Каждого ТекАппендер Из _НастройкиАппендеров Цикл
		...
		Для Каждого ПараметрАппендера Из ТекАппендер.Значение Цикл
			Если ПараметрАппендера.Ключ = "type" Тогда
				Результат.Добавить(ИмяАппендера + "=" + ПараметрАппендера.Значение);
			Иначе
				Результат.Добавить(ИмяАппендера + "." + ПараметрАппендера.Ключ + "=" + ПараметрАппендера.Значение);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
